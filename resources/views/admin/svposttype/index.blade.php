   ...
   <div class="row">
	...
	<div class="card">
	    <div class="card-body">
			...
			@endif
			<div align="right">
				<a class="btn btn-default btn-primary" href="{{ route('admin.svposttype.create') }}">Thêm mới</a>
			</div>
	            ...
	            <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
	                ...
	                <tbody>
	                	@foreach($svposttypes as $row)
							<tr>
								<td>{{ $row['name'] }}</td>						
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\SvPostTypeController@edit',$row['id_post_type']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\SvPostTypeController@destroy', $row['id_post_type'])}}">
								      {{csrf_field()}}
